...
print("Server running on: ", (SERVER_IP))                       # In ra địa chỉ server
...
def read_Request (Client):   # Hàm đọc yêu cầu từ client
    ...
    try:
        ...
        print("Error !!!, Cannot find any request from client")
    finally:
        return request
...
def take_Request (Client):   # Hàm nhận yêu cầu từ client
    while True:
        ...
        if "screenCapture" == Request:
            ...
            Client.sendall(bytes("Exit", "utf-8"))                                        # Gửi thông báo đã thoát
            break                                                                           
            ...
            Client.send(bytes("Task Killed", "utf-8"))    # Gửi thông báo đã xóa
        
        elif "HookKey" == Request:                                                            # Hook key
            Client.sendall(bytes("Đã nhận", "utf-8"))                                         # Gửi thông báo đã nhận
            ServerConfig.Keystroke(Client)                                                    # Gọi hàm Keystroke
...
def Waiting():    # Hàm chờ kết nối
    ...
    while True:
        ...
        #accept(): Phương thức này chấp nhận một cách thụ động kết nối TCP Client, đợi cho tới khi kết nối tới.
        print("Client:", Address, " Connected successfully !!!")                                         
...
def Listen():
    try:
        ...
        print("Error !!!, Server has stopped")
    finally:
        SERVER.close()          # Đóng socket

def main():   # Giao diện
    top = Tk()                                                           # Tạo cửa sổ
    top.title("Server")                                       # Đặt tiêu đề
    top.geometry("270x90")                                               # Đặt kích thước
    top.configure(bg="#7e5a5c")
    btn1= PhotoImage(file='./img/button/Openserver.png')                      # Đặt hình ảnh
    btn2= PhotoImage(file='./img/button/OpenserverPressed.png')
    def on_enter(event):
        top.button.config(image=btn2)   # Hình hiển thị khi chưa hover                                    
    def on_leave(event):
        top.button.config(image=btn1)   # Hình hiển thị khi hover

    top.button = Button(top, image=btn1,bg="#fff", command = Listen, relief="flat", bd=0, highlightthickness=0, activebackground="#f7f7f7")
