...
def screenCaptureFunction(self, client): 
	...
	#Tạo button Chụp ảnh	
		self.buttonTakePicture= PhotoImage(file='./img/button/TakePicture.png')                      # Đặt hình ảnh
		self.cap = Button(self.Screenshot, image=self.buttonTakePicture, command = ReceivePicture, bd = 0) #Nút chụp hình
		self.cap.place(relx=0, rely=0.88)
	#Tạo button Lưu ảnh
		self.buttonSave= PhotoImage(file='./img/button/Save.png')                      # Đặt hình ảnh
		self.Save = Button(self.Screenshot, image=self.buttonSave, command=SaveScreenshot, bd = 0)#Nút luu ảnh
		self.Save.place(relx=0.4532, rely=0.88)
	#Tạo button Không lưu ảnh
		self.buttonDSave= PhotoImage(file='./img/button/NotSave.png')                      # Đặt hình ảnh
		self.DontSave = Button(self.Screenshot, image=self.buttonDSave, command=DontSaveScreenshot, bd = 0)#Nút khong luu ảnh
